...
// ros related
ros::Publisher _wp_traj_vis_pub, _wp_path_vis_pub, _polynomial_traj_coef_pub;
...
{
    pose_msg = msg;
    _startPos << msg.pose.position.x, msg.pose.position.y, 0.0;
}
...
{
    vel_msg = msg;
    _startVel << msg.twist.linear.x, msg.twist.linear.y, 0.0;
}
...
void rcvWaypointsCallBack(const nav_msgs::Path &wp)
{
    ...
    {
        Vector3d pt(wp.poses[k].pose.position.x, wp.poses[k].pose.position.y, 0.0);
...
void trajGeneration(Eigen::MatrixXd path)
{
    ...
    ROS_INFO("Minisnap cost time %.4fms", duration.toSec() * 1000.0);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber _way_pts_sub = nh.subscribe("waypoints", 1, rcvWaypointsCallBack);
    ...
    _polynomial_traj_coef_pub = nh.advertise<minimumsnap::PolynomialTrajectory>("polynomial_traj_coef", 1);

    while (pose_msg.header.seq == 0 || vel_msg.header.seq == 0)
        ros::spinOnce();
...
VectorXd timeAllocation(MatrixXd Path)
{
    ...
    */
    double mid_num1 = _Vel * _Vel / _Acc;
    double mid_num2 = 2.0 * _Vel / _Acc;
    for (int i = 0; i < time.size(); i++)
    {
        double length = (Path.row(i) - Path.row(i + 1)).norm();
        if (length < mid_num1)
        {
            time(i) = sqrt(length / _Acc);
        }
        else
        {
            time(i) = mid_num2 + (length - mid_num1) / _Vel;
        }
    }
